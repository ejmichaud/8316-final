...
def run(width,
    ...
    all_dataset = torch.utils.data.Subset(all_dataset, np.random.choice(len(all_dataset), D, replace=False))
    data_train, data_test = torch.utils.data.random_split(all_dataset, [int(split*D), D-int(split*D)])
    ...
    for epoch in tqdm(range(epochs), desc='Epochs', disable=not verbose):
        ...
        with torch.no_grad():
            ...
            ex.info['train_accuracies'].append(acc)
            ...
            ex.info['test_accuracies'].append(acc)
